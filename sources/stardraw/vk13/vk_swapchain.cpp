...
vk_swapchain::vk_swapchain(vk_device& deviceRef, VkExtent2D extent, std::shared_ptr<vk_swapchain> previous)
    : device{ deviceRef }, window_extent{ extent }, old_swapchain{ previous } {
    init();
...
VkResult vk_swapchain::acquire_next_image(uint32_t* image_index) {
    vkWaitForFences(
        device.get_device(),
        1,
...
VkResult vk_swapchain::submit_command_buffers(
    ...
    images_in_flight[*image_index] = images_in_flight[current_frame];
    ...
    if (vkQueueSubmit(device.get_graphics_queue(), 1, &submitInfo, in_flight_fences[current_frame]) !=
        VK_SUCCESS) {
...
void vk_swapchain::create_swapchain() {
    ...
    uint32_t family_indices[] = { indices.graphics_family, indices.present_family };
    ...
    auto result = vkCreateSwapchainKHR(device.get_device(), &create_info, nullptr, &swapchain);
...
void vk_swapchain::create_image_views() {
    ...
    for (size_t i = 0; i < swapchain_images.size(); i++) {
        ...
        if (vkCreateImageView(device.get_device(), &view_info, nullptr, &swapchain_image_views[i]) !=
            VK_SUCCESS) {
...
void vk_swapchain::create_render_pass() {
    ...
    color_attachment.format = get_swapchain_image_formats();
...
void vk_swapchain::create_framebuffers() {
    ...
    for (size_t i = 0; i < image_count(); i++) {
        ...
        VkExtent2D swap_chain_extent = get_swapchain_extent();
...
VkFormat vk_swapchain::find_depth_format() {
